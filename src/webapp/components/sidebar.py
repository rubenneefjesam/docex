...
def render_sidebar(default_assistant: str = "general_support",
    ...
    # Buiten assistenten-mode: behoud state
    if main_menu != "Assistenten":
        st.session_state.setdefault("assistant_key", default_assistant)
        st.session_state.setdefault("tool_key", default_tool or "")
    ...
    if "assistant_key" not in st.session_state or "tool_key" not in st.session_state:
        ...
        st.session_state.tool_key = (
            t if t in ASSISTANTS[st.session_state.assistant_key]["tools"] else ""
        )
