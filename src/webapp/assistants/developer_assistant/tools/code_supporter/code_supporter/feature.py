...
INSTRUCTIONS = (
    ...
    "- Gebruik generieke IT-terminologie; geen domeinvoorbeelden.\n"
    "- Zorg dat 'To do' 1 tot maximaal 10 genummerde stappen bevat.\n"
)
...
# Reuse helper functions from story.py pattern (duplicated for module isolation)
...
def _has_minimum_structure(text: str) -> bool:
    low = text.lower()
    required = ["titel", "type", "beschrijving", "to do", "scope", "dependencies", "background"]
...
def _truncate_todo_steps(text: str, max_steps: int = 10) -> str:
    # same naive truncation as story.py
    import re as _re
    low = text.lower()
...
def _local_fill(template: str, short_input: str) -> str:
    def repl(m):
        ...
        if key in ("title", "titel"):
            return first
        if key == "description":
            return "Korte beschrijving van de feature: generieke functionaliteit die X mogelijk maakt."
        if key in ("to_do", "todo", "steps"):
            return "1. Definieer acceptance checklist\n2. Maak mock data en fixtures\n3. Implementeer feature API / UI\n4. Schrijf unit tests"
        if key == "scope":
            return "- Dekt core flows\n- Excludes deep integrations"
        if key in ("dependencies", "blockers"):
            return "- API contract voor retrieval\n- CI pipeline"
        if key == "background":
            return "Feature bundelt meerdere user stories en zorgt voor een herbruikbare capability. Focus op generieke oplossingen en testbaarheid."
        return first
        ...
        try:
            try:
                resp = client.chat.completions.create(
                    ...
                    max_tokens=1200,
                    messages=[
                        {"role": "system", "content": "Je bent een beknopte assistant voor het genereren van generieke IT features."},
