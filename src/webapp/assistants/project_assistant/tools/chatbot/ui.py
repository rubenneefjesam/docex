...
# Ensure this folder is importable when running streamlit from repo root
...
def run():
    ...
    up = st.sidebar.file_uploader("Upload document (.docx of .txt)", type=["docx", "txt"], key="up_files", accept_multiple_files=True)
    st.sidebar.markdown("---")
    ...
    if up and st.button("Ingest bestanden"):
        ...
        for f in up:
            ...
            if not cid or not pid:
                ...
                continue
            metas = [
                {"text": c, "client_id": cid, "project_id": pid, "filename": f.name, "chunk_index": i}
            ...
            if rows and emb_arr is not None:
                import numpy as _np
                save_index(cid, pid, rows + metas, _np.vstack([emb_arr, _np.array(embs, dtype=_np.float32)]).tolist())
            else:
    ...
    if st.button("Vraag stellen") and q.strip():
        with st.spinner("Zoeken en genereren…"):
            ...
            if not results:
                ...
                for r in results:
                    st.write(f"- {r['filename']} — chunk {r['chunk_index']} (score={r.get('_score', 0):.3f})")
